...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="writeCssFile">
        <xsl:result-document href="{testng:absolutePath('style.css')}" format="text">
            ...
            .suiteStatusPass, .suiteStatusFail, .suiteStatusSkip { padding-right: 20px; width: 20px; height: 20px; margin: 2px 4px 2px 2px; display: inline; }
            .suiteStatusPass { background-color: green; }
            .suiteStatusFail { background-color: red; }
            .suiteStatusSkip { background-color: Yellow; }
    ...
    <xsl:template name="htmlHead">
        <head>
            <title>
                <xsl:value-of select="testng:getVariableSafe($testNgXslt.reportTitle, 'TestNG Results (PROD ENV)')"/>
            </title>
    ...
    <xsl:function name="testng:suiteStateClass">
        ...
        <xsl:value-of select="if (count($testCaseElements/class/test-method[(@status='FAIL') and (not(@is-config))]) > 0) then 'suiteStatusFail' else if (count($testCaseElements/class/test-method[(@status='PASS') and (not(@is-config))]) > 0) then 'suiteStatusPass' else 'suiteStatusSkip'"/>
    </xsl:function>
        ...
        <xsl:for-each select="$testCases">
            ...
            <div class="testCaseLink"
                ...
                <div class="{if (count(./class/test-method[@status='FAIL']) > 0)
                                then 'testCaseFail'
                                else if ((count(./class/test-method[@status='FAIL']) = 0) and (count(./class/test-method[@status='PASS' and @name!='checkSuiteToRun']) > 0))
                                    then 'testCasePass'
                                    else 'testCaseSkip'}">
                </div>
                <xsl:value-of select="@name"/>
