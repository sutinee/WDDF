...
//Yvette: There is a project in NRS 65543 to decommission H&R Block/Ameriprise. RE: Test Result PriorDay Testing 19Sep2017
...
public class SuiteECCCase4_Ameriprise extends SuiteECCBase{
	...
	public void SuiteECCCase4_AmeripriseSDataTest(String DataURL, String DataBrowser, String DataUsername, String DataPassword, String DataAccountNumber, String DataBeginDate) throws InterruptedException, IOException{
		...
		wait.until(ExpectedConditions.textToBePresentInElement(driver.findElement(By.xpath("html/body/center/table[2]/tbody/tr/td/table/tbody/tr[1]/td/font/b")), "Click on date to View the Tax:"));
		...
		if(!statementList.contains("20")) // Check for statement of year 20xx
		{
			s_assert.assertTrue(statementList.contains("20"), "Tax Form NOT FOUND");
			Add_Log.info("Tax Form NOT FOUND");
        	Testfail = true;
		}
		
		// Click and link e.g. the 1st link
		...
		Add_Log.info("Tax form is displayed. Click one of tax form");
		try {
		...
		if(currentURL.contains(".onlinefinancialdocs")) // current URL on Chrome and FF are the same.
		{
			    ...
			    if(PDFContent.contains("1943"))
			    {
			    	fileName1_taxform = takeScreenshot(TestCaseName, "1_ViewTaxForm");
					Add_Log.info("PDF's content is correct. As expected");
			    }
			    else
			    {
			    	Add_Log.info("PDF content does not contain expected account no: 1943");
			    	Testfail = true;
			    	s_assert.assertTrue(PDFContent.contains("1943"), "PDF content does not contain expected account no: 1943");
			    	fileName1_taxform = takeScreenshot(TestCaseName, "1_ViewTaxFormFail");
			    }*/
				String PDFContent = "";
				String expectedWording = "1943";
		...
		reportOutput(fileName1_taxform, "1. View tax form", "1. Error occurs on Tax Form");
